%% 依固定时间收敛扰动观测器(Disturbance observer in fixed time)状态方程(State Equation)
% 作用：将输入的这些量转化成状态量导数值输出
% 输入参数：xi(2*2 辅助状态量) eta(2*2:z theta)、V_hat(2*2:w_hat q_hat)、m_b(2*2:m_bb m_sb)、q_b(2*2:q_bb q_sb)、tau_cp(2*2:Z_cp M_cp)、tau_VBS(2*2)、tau_w_hat(2*2)
% 输入参数共计 8 * 2 = 16
% 输出参数：xi_dot(2*2)
% 运行此函数需要运行参数化脚本 REMUS_All_Para_Init & DistObsv_All_Para_Init

function output =  DistbObsv_FixedTime_StateEqu(input)
    %% 输入量采集
    % xi: 2*2 辅助状态量
    xi = zeros(2,1);
    for i = 1:2
    ...
    % 辅助状态量书写
    varrho = M * (V_hat - xi);
    output = M \ (M_dot * (V_hat - xi) + tau_w_hat + K_4 * (PublicFunc_sig(0.5, varrho) + miu * PublicFunc_sig(1.5, varrho)) - C * V_hat - D * V_hat - g_eta + tau_cp + tau_VBS);
